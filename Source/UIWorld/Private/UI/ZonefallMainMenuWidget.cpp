...
void UZonefallMainMenuWidget::BuildLayout()
{
	...
	HeroTitle->SetShadowOffset(FVector2D(2.0f, 2.0f));
	HeroTitle->SetShadowColorAndOpacity(FLinearColor(0, 0, 0, 0.8f));
...
void UZonefallMainMenuWidget::UpdateOnlineIndicator()
{
	...
	FText StatusLabel = bLoggedIn
		? (Loc ? Loc->GetText(TEXT("online.status.online")) : FText::FromString(TEXT("ONLINE")))
		: (Loc ? Loc->GetText(TEXT("online.status.offline")) : FText::FromString(TEXT("OFFLINE")));

	FString Display = StatusLabel.ToString();
	if (GI)
	{
		...
		{
			Display = FString::Printf(TEXT("%s  •  %s (%s)"), *StatusLabel.ToString(), *Nick, *Service);
		}
		else
		{
			Display = FString::Printf(TEXT("%s  •  %s"), *StatusLabel.ToString(), *Service);
		}
	}
	...
	if (OnlineDot)
	{
		const FLinearColor DotColor = bLoggedIn
			? FLinearColor(0.30f, 0.92f, 0.45f, 1.0f)   // green = signed in
			: (bAvailable ? FLinearColor(0.96f, 0.78f, 0.30f, 1.0f)  // amber = service up, not signed in
						  : FLinearColor(0.65f, 0.65f, 0.65f, 1.0f)); // grey = offline
		OnlineDot->SetBrush(FSlateRoundedBoxBrush(DotColor, 8.0f, FVector2D(14.0f, 14.0f)));
	}
}
...
void UZonefallMainMenuWidget::PerformAction(EZonefallMenuAction Action)
{
	...
	case EZonefallMenuAction::PlayStory:
		if (GI) { GI->LoadLevelWithLoadingScreen(StoryLevelName, true); }
		break;
