...
void UZonefallOnlineLobbyWidget::NativeConstruct()
{
	...
	if (UUIWorldMenuGameInstance* GI = ResolveGameInstance())
	{
		if (PlayerBannerText)
		{
			...
			PlayerBannerText->SetText(FText::FromString(
				Nick.IsEmpty()
					? FString::Printf(TEXT("Build %s"), *GI->OnlineGameBuildId)
					: FString::Printf(TEXT("%s  |  Build %s"), *Nick, *GI->OnlineGameBuildId)));
		}
	}
...
void UZonefallOnlineLobbyWidget::BuildLayout()
{
	...
	PlayerBannerText = MakeText(
		...
		BodyFontSize - 1,
		AccentColor * 0.85f);
	if (UVerticalBoxSlot* BannerSlot = MainBox->AddChildToVerticalBox(PlayerBannerText))
	{
	...
	MapSelectBox = WidgetTree->ConstructWidget<UComboBoxString>(UComboBoxString::StaticClass(), TEXT("MapSelectBox"));
	{
		...
		MapSelectBox->Font = MakeFont(BodyFontSize);
	...
	MaxPlayersBox->Font = MakeFont(BodyFontSize);
...
void UZonefallOnlineLobbyWidget::RebuildSessionList(const TArray<FUIWorldOnlineSessionResult>& Results)
{
	...
	if (Results.Num() == 0)
	{
		UTextBlock* Empty = MakeText(WidgetTree, TEXT("EmptyLabel"),
			NSLOCTEXT("ZonefallLobby", "Empty", "No sessions found. Try host or refresh."),
			BodyFontSize, ZGlass::TextSecondary);
		SessionList->AddChild(Empty);
		if (UScrollBoxSlot* EmptySlot = Cast<UScrollBoxSlot>(Empty->Slot))
		{
			EmptySlot->SetPadding(FMargin(14, 18, 14, 18));
...
void UZonefallOnlineLobbyWidget::HandleSessionsFound(const TArray<FUIWorldOnlineSessionResult>& Results)
{
	...
	if (UUIWorldMenuGameInstance* GI = ResolveGameInstance())
	{
		...
		{
			const FString Diag = GI->GetLastOnlineDiagnostic();
			SetStatusMessage(Diag.IsEmpty() ? TEXT("No sessions found.") : Diag, false);
		}
		else
		{
